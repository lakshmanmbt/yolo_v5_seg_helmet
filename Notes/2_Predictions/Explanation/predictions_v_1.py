...
def run(
    ...
    # print("printing save_img",save_img)
    is_file = Path(source).suffix[1:] in (IMG_FORMATS)
    # print("printing is_file",is_file)
    is_url = source.lower().startswith(('rtsp://', 'rtmp://', 'http://', 'https://'))
    # print("printing is_url",is_url)
    webcam = source.isnumeric() or source.endswith('.streams') or (is_url and not is_file)
    # print("printing webcam",webcam)
    screenshot = source.lower().startswith('screen')
    # print("printing screenshot",screenshot)
    

    # Directories
    save_dir = increment_path(Path(project) / name, exist_ok=exist_ok)  # increment run
    # print("printing save_dir",save_dir)
    (save_dir / 'labels' if save_txt else save_dir).mkdir(parents=True, exist_ok=True)  # make dir

    # Load model
    device = torch.device('cpu')
    # print("printing device", type(device))
    model = DetectMultiBackend(weights, device=device, dnn=dnn, data=data, fp16=half)
    # print("printing model",model)
    stride, names, pt = model.stride, model.names, model.pt
    # print("printing pt",pt)
    imgsz = [640, 640]

    # Dataloader
    bs = 1  # batch_size
    dataset = LoadImages(source, img_size=imgsz, stride=stride, auto=pt, vid_stride=vid_stride)
    # print("printing len of dataset",len(dataset))
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        with dt[1]:
            ...
            # print("printing pred",pred.shape)

        # NMS
        with dt[2]:
            pred = non_max_suppression(pred, conf_thres, iou_thres, classes, agnostic_nms, max_det=max_det, nm=32)
            # print("printing pred",type(pred))
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default=ROOT / '2_Predictions/00057.jpg', help='file/dir/URL/glob/screen/0(webcam)')
