...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        with dt[0]:
            print("printing im shape",im.shape)
            print("printing im type",type(im))
            # ----------------------------------------------
            # cv2.imshow('im_np image', im)
            # cv2.waitKey(0)
            # cv2.destroyAllWindows()
            # ----------------------------------------------  
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default=ROOT / '2_Predictions/00069.jpg', help='file/dir/URL/glob/screen/0(webcam)')
